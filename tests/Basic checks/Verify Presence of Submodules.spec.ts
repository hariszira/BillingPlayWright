...
test.describe("Login to Billing Module and Verify Presence od Submodules", () => {
  test.beforeEach(async ({ loginPage }) => {
    await loginPage.login();
  });
  test('Verify Presence of Submodules', async ({ page }) => {
  
    // Now that we're logged in, we can proceed with the test actions
    ...
    // Check the presence of submodules
    const sections = [
      "Charge Management", 
      "Billing Profile", 
      "Approval", 
